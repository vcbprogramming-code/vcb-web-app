...
- ปุ่ม "×" ปิดแบนเนอร์ เรียก `dismissBanner()` ใน `App.jsx` → เซ็ต `bannerDismissed = true`
  และเรียก `markDismissed(announcement.id)` เพื่อจำไว้ที่ระดับเครื่อง (ดูหัวข้อ 3.10)
...
### 3.10 การจำสถานะ "ปิดแบนเนอร์แล้ว" ระดับเครื่อง — `src/lib/announcementDismissal.js`

**ทำอะไรได้:** จำไว้ว่าเครื่องนี้เคยกดปิดแบนเนอร์ประกาศฉบับไหนไปแล้ว เพื่อไม่ให้ประกาศเดิม
เด้งขึ้นมาซ้ำทุกครั้งที่รีเฟรชหน้า แต่ประกาศ**ฉบับใหม่**ต้องแสดงเสมอ สถานะนี้เก็บใน
`localStorage` ของเบราว์เซอร์แต่ละเครื่อง (key `vcb_portal_ann_dismissed`) ไม่ได้เก็บใน
ฐานข้อมูล จึงเป็นค่าเฉพาะเครื่อง/เฉพาะเบราว์เซอร์ ไม่ตามผู้ใช้ข้ามอุปกรณ์

**ฟังก์ชันที่ export ทั้ง 4 ตัว:**

- **`readDismissedId()`** — อ่านค่า id ที่เก็บไว้ คืนเป็น **string หรือ `null` เท่านั้น** ไม่เคย
  แปลงเป็นตัวเลข ทำ 3 อย่างในการเรียกครั้งเดียว: (1) **กวาดล้าง key เก่า** จากเวอร์ชันก่อนพอร์ต
  (`vcb_connect_ann_dismissed`) ทิ้งถ้ายังค้างอยู่ — ค่าในนั้นเป็น uuid ที่ไม่มีทางตรงกับ
  `revision` อยู่แล้ว จึงไม่มีอะไรเสียหายจากการลบ (2) อ่านค่าจาก key ปัจจุบัน (3) ป้องกันค่าที่
  เสียหายหรือถูกแก้ด้วยมือ — อะไรที่ไม่ใช่ string ที่ไม่ว่าง ถือว่า "ไม่มีค่าเก็บไว้"
- **`isDismissed(id)`** — ตอบว่าเครื่องนี้เคยปิดประกาศ id นี้ไปแล้วหรือยัง เทียบแบบ
  `stored === String(id)` คือ **string ล้วน ๆ** โดยเจตนา (ดูเหตุผลในหัวข้อ 5 ข้อ 7) ถ้า `id` เป็น
  `null`/`undefined` คืน `false` ทันที ใช้ที่ `App.jsx` ทุกครั้งที่โหลดประกาศมาใหม่ เพื่อตั้งค่า
  `bannerDismissed` เริ่มต้น
- **`markDismissed(id)`** — บันทึกว่าปิดประกาศ id นี้แล้ว เรียกจาก `dismissBanner()` ใน
  `App.jsx` ตอนผู้ใช้กดปุ่ม "×" บนแบนเนอร์
- **`clearDismissed()`** — ลบค่าที่เก็บไว้ทิ้ง ทำให้แบนเนอร์ฉบับปัจจุบันกลับมาแสดงอีกครั้งบน
  เครื่องนี้ **ปัจจุบันยังไม่มีจุดใดในโค้ด Portal เรียกใช้** เป็นฟังก์ชันที่เตรียมไว้สำหรับปุ่ม
  "รีเซ็ตการแจ้งเตือน" ในเมนูตั้งค่า หรือใช้ตอน sign-out ในอนาคต

**เหตุผลที่ทุกฟังก์ชันห่อด้วย `try/catch`:** `localStorage` สามารถ **throw ได้จริง** ในบางสภาพ
แวดล้อม (หน้าต่างส่วนตัวของ Safari, เบราว์เซอร์ที่ตั้งค่าบล็อก site data) การอ่าน/เขียนล้มเหลว
จะคืน `null` หรือเงียบไปเฉย ๆ — เรื่องเล็กอย่าง "จำไม่ได้ว่าเคยปิดแบนเนอร์" ต้องไม่มีทางทำให้ทั้ง
หน้าจอพัง อย่างมากที่สุดคือแบนเนอร์กลับมาแสดงใหม่ตอนรีโหลด

### 3.11 การจัดการประกาศ (Admin) — `src/AnnouncementEditor.jsx`
...
- คำแปลของแต่ละ tile (ชื่อ/คำอธิบาย/preview) มี logic ลำดับความสำคัญพิเศษ ดูหัวขัด 3.13

### 3.13 การ Resolve ข้อความของ tile — `src/lib/appCopy.js`
...
### 3.14 การจัดการ tile แอปพลิเคชัน (Admin) — `src/lib/portalApi.js`

**ทำอะไรได้:** เพิ่ม/แก้ไข/ลบ/จัดลำดับ tile ที่แสดงในกริดแอปพลิเคชันและใน Sidebar โดยไม่ต้อง
แก้โค้ดและ redeploy SPA ใหม่ — เป็นเหตุผลเดียวกับที่หัวข้อ 3.13 ให้ค่าจากฐานข้อมูลชนะ dictionary
ท้องถิ่น: tile เป็นข้อมูล ไม่ใช่โค้ด

**สถานะปัจจุบัน (สำคัญ):** ฟังก์ชันทั้ง 4 ตัวนี้ถูก export จาก `portalApi.js` และ backend
รองรับครบแล้ว แต่ **ยังไม่มีคอมโพเนนต์ใดใน `portal/src/` import ไปใช้** (ตรวจสอบแล้วเมื่อ
2026-09-05 — ไม่มีหน้าจอจัดการ tile ในโค้ดปัจจุบัน) จึงเป็นชั้น wrapper ที่พร้อมใช้งานรอหน้าจอ
แอดมินที่ยังไม่ถูกสร้าง ปัจจุบันการเพิ่ม/แก้ tile ต้องทำผ่าน SQL ตรงที่ตาราง `portal.apps`

**สิทธิ์:** ทั้ง 4 endpoint บังคับ `requireAuth + requireRole('portal','admin')` ที่ server
(ตัวแปร `admin` ใน `api/src/routes/portal.js`) — เฉพาะ **portal admin** เท่านั้น ไม่มีการเช็ค
สิทธิ์ฝั่ง client ในตัว wrapper เลย ถ้าคนทั่วไปเรียกจะได้ HTTP 403 กลับมา

| ฟังก์ชัน | Endpoint | ทำอะไร |
|---|---|---|
| `createApp(app)` | `POST /api/portal/apps` | สร้าง tile ใหม่ คืน record ที่สร้างพร้อม HTTP 201 |
| `updateApp(key, patch)` | `PATCH /api/portal/apps/:key` | แก้ไขบางฟิลด์ของ tile เดิม (partial update) |
| `deleteApp(key)` | `DELETE /api/portal/apps/:key` | ลบ tile ถาวร คืน `{ok:true}` |
| `reorderApps(keys)` | `PUT /api/portal/apps/order` | จัดลำดับ tile ใหม่ทั้งชุดในคำสั่งเดียว |

**Logic การทำงานโดยละเอียด:**

- **`createApp(app)`** — ส่ง object ทั้งก้อนไป server ตรวจด้วย `appSchema` (Zod) ก่อนเสมอ
  ข้อบังคับที่สำคัญ:
  - `key` ต้องตรง regex `^[a-z0-9][a-z0-9_-]*$` (ตัวพิมพ์เล็ก ตัวเลข `-` `_` เท่านั้น, ยาวไม่เกิน
    40) เพราะ `key` เป็น primary key และเป็นตัวที่ client ใช้ค้นไอคอน (`AppIcon`) กับคำแปลใน
    `src/i18n.js` จึงต้องเป็น slug ไม่ใช่ข้อความอิสระ
  - `url` ต้องเป็น URL ที่ขึ้นต้นด้วย `http://` หรือ `https://` เท่านั้น — **เจตนากันไม่ให้บัญชี
    แอดมินที่ถูกยึดเปลี่ยน tile หน้าประตูบ้านของทั้งบริษัทให้เป็น `javascript:` payload ได้**
  - `accent` ต้องเป็นสี hex (`#` ตามด้วย 3–8 หลัก) ค่า default `#4fd1ff` — ใช้เป็นสีไอคอนของ tile
    (ดูหัวข้อ 3.5.3)
  - ฟิลด์ที่เหลือมี default: `nameTh`/`descriptionTh`/`icon` = ค่าว่าง, `sortOrder` = `0`,
    `enabled` = `true`
- **`updateApp(key, patch)`** — ใช้ `appSchema.omit({key:true}).partial()` คือ**แก้ `key` ไม่ได้**
  โดยเจตนา เพราะการเปลี่ยน key จะทำให้คำแปลใน dictionary และไอคอนที่ผูกกับ key เดิมกลายเป็น
  กำพร้าทันที ถ้าจำเป็นต้องเปลี่ยนต้องลบแล้วสร้างใหม่ ฟิลด์ที่ไม่ได้ส่งมาจะถูก `coalesce` กับค่าเดิม
  ใน SQL (ไม่ถูกล้างเป็นค่าว่าง) ถ้าไม่พบ key คืน `404 NOT_FOUND`
- **`deleteApp(key)`** — ลบแถวจริงออกจาก `portal.apps` (ต่างจากการล้างประกาศในหัวข้อ 3.11 ที่
  blank ข้อความแทนการลบแถว) ถ้าไม่พบ key คืน `404 NOT_FOUND`
- **`reorderApps(keys)`** — รับ **array ของ key เรียงตามลำดับที่ต้องการ** (สูงสุด 100 รายการ)
  แล้วเซ็ต `sort_order` ตาม **ตำแหน่งในอาร์เรย์** โดยตรง: server ใช้
  `unnest($1::text[]) with ordinality` ซึ่งให้เลขลำดับเริ่มที่ **1** (ไม่ใช่ 0) แล้ว update ทุกแถว
  ที่ key ตรงกันด้วย `UPDATE` คำสั่งเดียวภายใน transaction — ไม่ใช่ loop ยิง `PATCH` ทีละ tile
  เหตุผล: หน้าจอแอดมินที่ลาก tile จัดลำดับจะเปลี่ยนตำแหน่งหลายตัวพร้อมกัน การส่งคำสั่งเดียวทำให้
  ลำดับใหม่ทั้งชุด commit พร้อมกันหรือไม่ commit เลย ไม่มีสถานะกลางที่ลำดับซ้ำ/สลับกันชั่วขณะ
  - key ที่ไม่มีอยู่จริงในตารางจะถูกข้ามไปเงียบ ๆ (`where a.key = t.k` ไม่ match) และ tile ที่ไม่ได้
    อยู่ใน array เลยจะคง `sort_order` เดิมไว้
  - คืน `{ok:true, count}` โดย `count` คือจำนวน key ที่ส่งไป (ไม่ใช่จำนวนแถวที่แก้จริง)
- ลำดับสุดท้ายที่ผู้ใช้เห็นมาจาก `order by sort_order, name` ของ `GET /api/portal/apps` — ถ้า
  `sort_order` เท่ากัน (เช่น tile ใหม่ที่ยังไม่เคยจัดลำดับ ค่าเป็น `0` ทั้งหมด) จะเรียงตามชื่อแทน

### 3.15 คอมโพเนนต์ UI พื้นฐาน — `src/ui.jsx`

`TECH_STACK.md` กำหนดว่าโมดูลนี้**ไม่ใช้ UI library ภายนอก** คอมโพเนนต์พื้นฐานทั้งหมดจึงเขียนเอง
รวมไว้ในไฟล์เดียวคือ `src/ui.jsx` เหตุผลที่รวมไว้ที่เดียวแทนที่จะเขียน class string ซ้ำในทุกไฟล์
คือเพื่อกันไม่ให้ปุ่ม/ฟอร์มในแต่ละหน้าจอค่อย ๆ เพี้ยนออกจากกัน ทุกตัวรองรับธีมมืดในตัว (คู่ class
`dark:`) และ forward props ที่เหลือลง element จริงด้วย `{...rest}` จึงใส่ `onClick`, `disabled`,
`aria-*` ได้ตามปกติ

| คอมโพเนนต์ | หน้าที่ | ใช้ที่ไหน |
|---|---|---|
| `Button` | ปุ่มหลัก มี 4 variant: `default`, `primary`, `danger`, `ghostDanger` | `AnnouncementEditor.jsx`, `HelpModal.jsx` |
| `IconButton` | ปุ่มไอคอนสี่เหลี่ยมจัตุรัส 36×36px ไม่มีข้อความ | `Topbar.jsx` (เมนู/ตั้งค่า), `Dashboard.jsx` (ปิดแบนเนอร์), `HolidayCalendar.jsx` (ลูกศรเลื่อนเดือน) |
| `TextField` | ช่องกรอกข้อความบรรทัดเดียว พร้อม label ที่ผูก `htmlFor`/`id` ให้อัตโนมัติ | `AnnouncementEditor.jsx` (Title) |
| `TextArea` | ช่องกรอกหลายบรรทัด ปรับความสูงได้ (`resize-y`) ค่าเริ่มต้น 4 แถว | `AnnouncementEditor.jsx` (Body), `HelpModal.jsx` (รายละเอียดปัญหา) |
| `SelectField` | dropdown พร้อม label ใช้สไตล์เดียวกับช่องกรอกอื่น | `HelpModal.jsx` (เลือกหมวดปัญหา) |
| `Message` | ข้อความสถานะใต้ฟอร์ม 3 โทน: ปกติ / `ok` / `err` — **ไม่ render อะไรเลยเมื่อไม่มีข้อความ** เพื่อไม่ให้เลย์เอาต์กระตุกตอนข้อความปรากฏและหายไป และตั้ง `role="alert"` เมื่อเป็น error (กรณีอื่นเป็น `role="status"`) เพื่อให้ screen reader อ่านออก | `AnnouncementEditor.jsx`, `HelpModal.jsx` |
| `Modal` | กล่องโต้ตอบกลางจอบนฉากมืด ปิดได้ทั้งด้วย `Escape` และคลิกนอกกรอบ ล็อกไม่ให้หน้าหลังเลื่อน (`body.style.overflow`) และ **unmount ทิ้งเมื่อปิด** ไม่ใช่แค่ซ่อน เพื่อไม่ให้ปุ่มข้างในถูก tab เข้าถึงได้ตอนปิดอยู่ | `AnnouncementEditor.jsx`, `HelpModal.jsx` |
| `ModalTitle` | หัวข้อของ modal (`<h2>`) รับ `id` ไปผูกกับ `aria-labelledby` ของ `Modal` | เหมือนข้างบน |
| `ModalSub` | คำอธิบายรองใต้หัวข้อ modal | เหมือนข้างบน |
| `ModalActions` | แถวปุ่มท้าย modal จัดชิดขวา ตัดขึ้นบรรทัดใหม่ได้บนจอแคบ | เหมือนข้างบน |
| `Panel` | การ์ด (`<section>`) ที่เป็นพื้นผิวของทุกพาเนลใน Dashboard | `Dashboard.jsx`, `HolidayCalendar.jsx` |
| `PanelHead` | แถวหัวการ์ด จัดหัวข้อไว้ซ้าย ปุ่มหรือตัวเลขไว้ขวา | เหมือนข้างบน |
| `PanelTitle` | หัวข้อของการ์ด (`<h3>`) ตัวพิมพ์ใหญ่ ขนาดเล็ก | เหมือนข้างบน |

ไฟล์นี้ยัง export ค่าคงที่ class 2 ตัวให้ใช้ตรง ๆ เมื่อต้องการพื้นผิวหรือ label แบบเดียวกันแต่ไม่
ต้องการโครง element ของคอมโพเนนต์: `CARD_CLASS` (พื้นผิวการ์ด — `Dashboard.jsx` ใช้กับ element
ที่ไม่ใช่ `<section>`) และ `LABEL_CLASS` (สไตล์ label — `AnnouncementEditor.jsx` ใช้กับ checkbox
ที่ไม่เข้ากับ `TextField`)

### 3.16 ชุดไอคอน — `src/icons.jsx`

ไอคอนทั้งหมดเป็น **inline SVG แบบเส้น (line art)** ที่พอร์ตมาจาก `index.html` ของเวอร์ชัน Apps
Script โดยตรง ไม่ได้ใช้ไลบรารีไอคอนภายนอกและไม่โหลดไฟล์รูปเพิ่ม จึงไม่มี network request และ
เปลี่ยนสีตามธีมได้เองเพราะทุกตัวใช้ `stroke="currentColor"` เดิมขนาดไอคอนถูกกำหนดจาก CSS
descendant rule (เช่น `.sidebar-logo svg { width: 21px }`) แต่เมื่อย้ายมาใช้ Tailwind กฎเหล่านั้น
หายไป ทุกไอคอนจึงรับ prop `className` แทน โดยมีค่าเริ่มต้นเป็น `h-[18px] w-[18px]` — ผลคือ
**จุดที่เรียกใช้เป็นผู้กำหนดขนาด** และไอคอนตัวเดียวกันถูกนำไปใช้ซ้ำที่ขนาดต่างกันได้โดยไม่ต้อง
เพิ่ม CSS rule ใหม่

- **ไอคอนเมนูและแถบเครื่องมือ:** `MenuIcon` (เปิด sidebar บนมือถือ), `SearchIcon` (ช่องค้นหา),
  `GearIcon` (เมนูตั้งค่า), `HelpIcon` (ปุ่มช่วยเหลือ), `MailIcon`, `AnnouncementIcon`
  (หัวพาเนลและแบนเนอร์ประกาศ), `NavArrowIcon` (ลูกศร "เปิดใช้งาน →" บน tile — ค่าเริ่มต้นเล็กกว่า
  ตัวอื่นคือ `h-3.5 w-3.5`)
- **ไอคอนรายการใน Sidebar:** `DashboardIcon`, `OnboardingIcon`, `ErpIcon`, `ZoomIcon`,
  `AiTavernIcon` (ปุ่มที่ยัง disabled ถาวร), `GlobeIcon`
- **`AppIcon({icon})`** — ต่างจากตัวอื่นตรงที่**ไม่ใช่ไอคอนเดี่ยว** แต่เป็นตัวเลือกไอคอนตามค่า
  `icon` ของ record ในตาราง `portal.apps` ผ่าน `switch`: รองรับ `memo`, `minutes`, `sop`,
  `sysmap`, `hr`, `credit` และมี **default case เป็นไอคอนสี่เหลี่ยมมีเครื่องหมายบวก** สำหรับ tile
  ที่ค่า `icon` ว่างหรือเป็นค่าที่ยังไม่รู้จัก — จุดนี้สำคัญเพราะแอดมินเพิ่ม tile ใหม่ผ่าน
  `createApp()` (หัวข้อ 3.14) ได้โดยที่โค้ดยังไม่มีไอคอนเฉพาะให้ tile นั้น และ tile ต้องไม่แสดง
  เป็นช่องว่าง
...
| `POST` | `/api/portal/apps` | admin (`requireAuth` + `requireRole('portal','admin')`) | `portalApi.js` (`createApp`) — มี wrapper แล้วแต่ยังไม่มี UI เรียกใช้ ดูหัวข้อ 3.14 | tile ที่สร้างใหม่ |
| `PATCH` | `/api/portal/apps/:key` | admin | `portalApi.js` (`updateApp`) — ยังไม่มี UI เรียกใช้ ดูหัวข้อ 3.14 | tile ที่แก้ไข (key เปลี่ยนไม่ได้ — ต้องลบแล้วสร้างใหม่) |
| `DELETE` | `/api/portal/apps/:key` | admin | `portalApi.js` (`deleteApp`) — ยังไม่มี UI เรียกใช้ ดูหัวข้อ 3.14 | `{ok:true}` |
| `PUT` | `/api/portal/apps/order` | admin | `portalApi.js` (`reorderApps`) — ยังไม่มี UI เรียกใช้ ดูหัวข้อ 3.14 | จัดลำดับ tile ใหม่ทั้งชุดในคำสั่งเดียว (ใช้ `unnest` array แทนการ update ทีละแถว) |
...
7. **รหัสประกาศ (announcement id) เปลี่ยนรูปแบบ** — จาก uuid สุ่มใหม่ทุกครั้งที่บันทึก
   ...
   `isDismissed()` ใน `src/lib/announcementDismissal.js` เทียบค่าแบบ string ล้วน ๆ ไม่แปลงเป็น
   ตัวเลข จึงไม่มีทาง crash จากค่าเก่าที่ค้างอยู่ ระบบยังกวาดล้าง key เก่า
   (`vcb_connect_ann_dismissed`) ทิ้งตอนอ่านครั้งแรกด้วย (`readDismissedId()`) — รายละเอียด
   ของทั้งชุดอยู่ในหัวข้อ 3.10
...
10. ~~**`src/Globe.jsx` เป็นโค้ดที่ไม่ถูกเรียกใช้ (dead code)**~~ — **แก้ไขแล้ว: ลบไฟล์ทิ้งเมื่อ
    2026-09-05** (commit `656ee23`) ไฟล์นี้ (583 บรรทัด) วาดทรงกลมอนุภาคด้วย Fibonacci
    sphere sampling + เมทริกซ์หมุน 3 มิติบน canvas เป็นงานรุ่นก่อนที่ถูกแทนที่ไปแล้วและไม่มีไฟล์ใด
    import เลย

    **ยืนยันก่อนลบด้วยการ build จริง**: build โมดูล portal ทั้งก่อนและหลังลบ ได้บันเดิลขนาด
    **247,477 ไบต์เท่ากันทุกไบต์** พิสูจน์ว่า Vite ไม่เคยรวมไฟล์นี้เข้าบันเดิลตั้งแต่แรก การลบจึงไม่
    กระทบหน้าจอใด ๆ

    **ลูกโลกที่หน้า sign-in ใช้จริงคือ `src/GlobeBackground.jsx` ซึ่งเป็นคนละไฟล์และยังอยู่ครบ** —
    เป็นลูกโลกแบบ plexus network เต็มจอ (260 โหนดเชื่อมเส้นถึงกัน) พร้อมฟังก์ชัน `startLogin()`
    ที่ทำให้ลูกโลก "แตกกระจาย" ตอนกดเข้าสู่ระบบ ส่วน `GlobeIcon` ที่ `Sidebar.jsx` เรียกใช้มาจาก
    `src/icons.jsx` ไม่เกี่ยวกับทั้งสองไฟล์นี้
