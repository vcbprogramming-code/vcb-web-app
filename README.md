...
In Apps Script there is also no frontend/backend split: the backend is the
`api_*` functions and the frontend is the HTML served from the same file. That
is why those folders are flat rather than split into `frontend/` + `backend/`.
...
choice will not follow them between modules. Token names and palettes differ
too. `hr-worklog` has the most complete implementation; use it as the model when
converging.
