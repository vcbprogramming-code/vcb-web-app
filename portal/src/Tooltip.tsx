...
export interface TooltipContent {
  ...
  name: string
  desc: string
  /** 'nav' places the tooltip to the target's right; 'card' places it above. */
  kind: 'nav' | 'card'
}
...
export function useTooltip() {
  ...
  const currentRef = useRef<string | null>(null) // key of the content currently shown/showing
  ...
  const reveal = useCallback((el: HTMLElement, content: TooltipContent) => {
  ...
  const fadeOutThen = useCallback((next: () => void) => {
    ...
    window.setTimeout(next, FADE_OUT_MS)
  }, [])

  const show = useCallback(
    (el: HTMLElement, content: TooltipContent) => {
  ...
  const bind = useCallback(
    (content: TooltipContent) => ({
      onMouseEnter: (e: React.MouseEvent<HTMLElement>) => show(e.currentTarget, content),
      onMouseLeave: () => hide(content.key),
      onFocus: (e: React.FocusEvent<HTMLElement>) => show(e.currentTarget, content),
...
export default function Tooltip({ state }: TooltipProps) {
  ...
  return (
    ...
    >
      <span className="tt-name">{state.content?.name ?? ''}</span>
      <span className="tt-desc">{state.content?.desc ?? ''}</span>
    </div>
  )
}
