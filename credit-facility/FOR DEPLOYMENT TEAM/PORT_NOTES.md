...
**Last synced to GAS:** live deploy **@174** (`../ORIGINAL CODE/index.html`, `Code.js`, `Seed.js`
...
| GAS source (`../ORIGINAL CODE/`) | React file | How copied |
|-------------------|------------|------------|
...
2. **Markup changed?** Re-copy the `<body>` (header → toast) into
   `src/app/body.html`.
3. **App JS changed?** `sed -n '750,3938p' "../ORIGINAL CODE/index.html" > src/app/legacy.js`
   (adjust the line range if the script bounds moved — re-check with
   `grep -n '<style>\|</style>\|<body>\|<script>\|</script>' "../ORIGINAL CODE/index.html"` first).
