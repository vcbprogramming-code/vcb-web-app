...
This folder ("FOR DEPLOYMENT TEAM") is a **downstream mirror** of the
canonical app. Source of truth = the standalone Google Apps Script
`Index.html` (v8.86) + `Code.js` `doGet()` wrapper, now at
`E:\WORK\08 CLAUDE CODE\System Map App\ORIGINAL CODE\Index.html` (moved into
its own `ORIGINAL CODE` sibling folder to separate it cleanly from this React/
TypeScript port; this repo does not track a live `script.google.com`
deployment the way some sibling ports document one — the app is fully
self-contained in the one HTML file, and `Code.js`/`doGet()` is only
referenced here because that's the standard GAS wrapper pattern this file is
deployed with).
...
5. If the change affects actual business-process content (a lane, node,
   cross-connection, department, ERP module or function-registry row — not
   just code/rendering), also update the relevant section of
   `../COMPANY_OPERATING_MODEL.md`, since that file is a hand-authored
   plain-English mirror of `src/data/*.ts` for non-technical/other-tool
   consumption and does not regenerate itself.
