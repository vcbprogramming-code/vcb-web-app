...
**Rename and department-switch have no UI.** `useProgress` exposes
`renameEmployee` and `switchDepartment`, both fully wired to their endpoints,
but no page calls them — the original React scaffold had no UI for either
