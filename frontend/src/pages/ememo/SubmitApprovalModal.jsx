...
export default function SubmitApprovalModal({ documentId, onClose, onSubmitted }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg">
        ...
        <form onSubmit={submit} className="p-6 space-y-4">
          ...
          <div className="space-y-2">
            {approvers.map((a, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <input value={a.name} onChange={(e) => update(i, 'name', e.target.value)} placeholder="ชื่อ (ไม่บังคับ)" className={`${field} w-32`} />
                <input value={a.email} onChange={(e) => update(i, 'email', e.target.value)} placeholder="อีเมล *" type="email" className={`${field} flex-1`} />
                {approvers.length > 1 && (
          ...
          <button type="button" onClick={add} className="text-sm text-blue-600 hover:underline">+ เพิ่มผู้อนุมัติ</button>
