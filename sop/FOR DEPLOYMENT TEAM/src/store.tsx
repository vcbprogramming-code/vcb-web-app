...
/** null = closed, a number = editing that scenario's `no`, 'new' = create-case mode. */
export type EditorState = number | 'new' | null;
...
export interface Store {
  ...
  createScenario: (payload: ScenarioCreate) => Promise<void>;
  swapScenario: (payload: ScenarioSwap) => Promise<void>;
  deleteScenarioByNo: (no: number) => Promise<void>;
...
export function useStore(): Store {
  ...
  const closeEdit = useCallback(() => setEditor(null), []);

  /** Shared by save/create: follow the case to its (possibly new) module list
   * so an edit that changes the module doesn't make the case seem to vanish
   * from the list currently being viewed (mirrors doSave() in index.html). */
  const followToModule = useCallback((module: string | undefined, no: number | null) => {
    if (!module) return;
    setNav((n) => ({ ...n, view: 'sop', mod: module, sel: no }));
  }, []);
  ...
  const saveScenario = useCallback(
    async (payload: ScenarioEdit) => {
      ...
      setEditor(null);
      followToModule(payload.module, payload.no);
      await refreshClientData();
    },
    [refreshClientData, followToModule],
  );

  const createScenarioAction = useCallback(
    async (payload: ScenarioCreate) => {
      const result = await createScenario(payload);
      setEditor(null);
      followToModule(payload.module, result.no);
      await refreshClientData();
    },
    [refreshClientData, followToModule],
  );
  ...
  const deleteScenarioByNo = useCallback(
    async (no: number) => {
