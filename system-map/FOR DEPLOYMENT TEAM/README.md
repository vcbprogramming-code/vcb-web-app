...
For a plain-English, tool-agnostic description of the actual business
workflow this app maps (not the app's code) — every lane, node, department,
ERP module and function, written so it can be handed to any coding
assistant — see **`../COMPANY_OPERATING_MODEL.md`** at the repo root. It's
generated from this folder's `src/data/*.ts`; if that data changes materially,
regenerate the summary rather than letting it drift.
