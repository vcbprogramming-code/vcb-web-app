 ...
 * module; this only affects list membership. Mirrors caseInModule() in
 * index.html. */
function caseInModule(sc: Scenario, mod: string): boolean {
...
function ScenarioList({ s }: { s: Store }) {
  ...
  const rows = useMemo(() => {
    let filtered = s.scenarios.filter((sc) => {
    ...
    // cases by row position, which reads as broken numbering (stable sort
