...
10. **`src/Globe.jsx` (583 บรรทัด) เป็นโค้ดที่ไม่ถูกเรียกใช้จากที่ใดเลย (dead code)** — พบจาก
    การตรวจสอบ import ทั้งโมดูลเมื่อ 2026-09-05: ไม่มีไฟล์ใด import `./Globe` เลย หน้า
    `SignInScreen.jsx` ใช้ `GlobeBackground.jsx` (คนละไฟล์ เขียนขึ้นใหม่) แทน ส่วน `GlobeIcon`
    ที่ `Sidebar.jsx` เรียกใช้นั้นมาจาก `src/icons.jsx` ไม่เกี่ยวกับไฟล์นี้ — `Globe.jsx` วาดทรงกลม
    อนุภาคด้วย Fibonacci sphere sampling + เมทริกซ์หมุน 3 มิติบน canvas ซึ่งน่าจะเป็นรุ่นก่อนที่ถูก
    แทนที่ไปแล้ว **ทีมพัฒนาควรตัดสินใจว่าจะลบทิ้งหรือเก็บไว้** — ปัจจุบันมันถูกรวมเข้าไปในบันเดิล
    โดยเปล่าประโยชน์หากไม่มีใครตั้งใจใช้
