...
(`E:\WORK\08 CLAUDE CODE\System Map App\ORIGINAL CODE\Index.html`, v8.86).
This React folder ("FOR DEPLOYMENT TEAM") is a downstream mirror — see
`PORT_NOTES.md` for the sync history and file mapping.

## Business-process summary (2026-08-06)
`E:\WORK\08 CLAUDE CODE\System Map App\COMPANY_OPERATING_MODEL.md` (repo
root, one level up from this folder) is a standalone Markdown write-up of the
actual company workflow this app
represents — the 10 lanes / 79 nodes / 129 connections / 158-function
registry, described in plain English rather than as app data. It exists so
the business process can be handed to any other coding assistant or analyst
without them needing to read this codebase. It was hand-authored from
`src/data/*.ts` on 2026-08-06 and is **not** auto-regenerated — if the
underlying data changes materially (new lane/node/function, re-scoped ERP
module), re-derive the relevant section of that file by hand; there is no
build step that keeps it in sync automatically.
