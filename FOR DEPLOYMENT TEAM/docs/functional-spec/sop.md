...
| `src/components/TopBar.jsx` | แถบบนสุด: ชื่อโมดูล, ช่องค้นหาส่วนกลาง, ทางเข้าการตั้งค่า (ห่อ `AppBar` ร่วม) |
| `src/components/Welcome.jsx` | หน้าต้อนรับ/หน้าเริ่มต้นเมื่อยังไม่ได้เลือก case หรือ flow |
| `src/components/LoadError.jsx` | หน้าจอเมื่อโหลดเอกสารไม่สำเร็จ + ปุ่มลองใหม่ |
| `src/components/ui.jsx` | คอนโทรลพื้นฐานที่ใช้ซ้ำ: Button, Modal, Field, Notice, Empty, Spinner, BackBar |
| `src/i18n/dict.js` | พจนานุกรมคำแปลไทย/อังกฤษของทั้งโมดูล (คีย์แบน `{th, en}`) |
| `src/lib/icons.jsx` | ชุดไอคอน inline SVG (`<Icon name="…" />`) ฝังในแอป ไม่พึ่ง CDN |
...
- **การจัดการ error แบบ "ห้ามดูเหมือนบันทึกสำเร็จ"** (หลักการสำคัญของทั้งโมดูล ดูหัวข้อ 5.3): ทุก mutation เป็น
...
ที่แอปต้นฉบับไม่มี — ดูหัวข้อ 7) มีปุ่มลิงก์ไปยัง NotebookLM ภายนอกด้วย
...
- หน้านี้เป็นฟีเจอร์ที่ **เพิ่มใหม่ในระบบพอร์ต** (ไม่มีในแอปต้นฉบับ Apps Script) — ดูรายละเอียดในหัวข้อ 7
...
### 2.9 แถบบนสุด (TopBar) — ชื่อโมดูล, ช่องค้นหา, ทางเข้าการตั้งค่า

**ทำอะไรได้:** `src/components/TopBar.jsx` คือแถบสีแบรนด์ที่อยู่บนสุดของทุกหน้าจอ ประกอบด้วย 3 ส่วน:

- **ชื่อโมดูลและคำบรรยายใต้ชื่อ** — `title={t('app.title')}` ("ระเบียบปฏิบัติงานมาตรฐาน") และ
  `subtitle={t('app.subtitle')}` ("มาตรฐานการใช้งานระบบ")
- **ช่องค้นหาส่วนกลาง** — ช่องเดียวที่ใช้ค้นได้ทั้ง Case Studies, Process Flows และ Reports (แต่ละหน้าไปอ่าน
  ค่าเดียวกันนี้จาก store เอง ดูหัวข้อ 2.2 และ 2.6)
- **ทางเข้าการตั้งค่า** — รายการ "ตั้งค่า" ในชีตการตั้งค่าของ `AppBar` ซึ่งเปิด `SettingsModal` ของโมดูลนี้ต่อ
  (ดูหัวข้อ 2.8)

**Logic การทำงาน:**

- คอมโพเนนต์นี้ **ไม่ได้เขียนแถบขึ้นมาเองทั้งแถบ** แต่ห่อ `AppBar` จาก `@vcb/shared` ไว้ — คอมเมนต์หัวไฟล์บันทึก
  ที่มาไว้ชัดเจนว่าเดิมเป็น header ที่ก๊อปมาเขียนมือแล้วค่อย ๆ เพี้ยนออกจากโมดูลอื่น (ไอคอนเฟือง 36px ในขณะที่
  โมดูลอื่นเป็น 32px, ไม่ sticky, และปักอีเมลผู้ล็อกอินไว้บนแถบซึ่งไม่มีโมดูลไหนทำ) จึงเปลี่ยนมาใช้คอมโพเนนต์ร่วม
  แทน — *"Copies drift; this is the component."*
- **props:** รับเพียงตัวเดียวคือ `onSettings` (callback เปิด SettingsModal) ถ้าไม่ส่งมา ปุ่ม "ตั้งค่า"
  (`settingsExtra`) จะไม่ถูก render เลย (เงื่อนไข `onSettings ? (…) : null`) แถบยังทำงานได้ตามปกติ ไม่พัง
- **ช่องค้นหาไม่มี state ของตัวเอง** — ผูกกับ store โดยตรง (`const { query, setQuery } = useStore()`)
  เป็น controlled input ที่ `onChange` ยิง `setQuery(e.target.value)` ทุกตัวอักษร การค้นหาจึงเป็น live filter
  ไม่มีปุ่ม "ค้นหา" และไม่มี debounce เพราะการกรองทั้งหมดทำในหน่วยความจำฝั่ง client
- **ขนาดตัวอักษรของช่องค้นหาถูกบังคับไว้ที่ `text-base` (≥16px) บนจอแคบ** แล้วค่อยลดเป็น `lg:text-sm` บนจอกว้าง
  — คอมเมนต์ในโค้ดระบุเหตุผลไว้ว่า iOS จะซูมหน้าเว็บอัตโนมัติเมื่อโฟกัสช่องกรอกที่ตัวอักษรเล็กกว่า 16px
- **การแสดงตัวตนผู้ใช้ถูกย้ายออกจากแถบ** ไปอยู่ท้ายชีตการตั้งค่า ตำแหน่งเดียวกับ HR, Credit Facility และ
  Meeting Minutes สิ่งที่ยังคงอยู่ในไฟล์นี้คือส่วนที่เป็นลักษณะเฉพาะของโมดูล SOP เท่านั้น คือ 2 ค่า:
  - `identityNote` — ป้าย "ผู้แก้ไข" (`t('auth.editorBadge')`) แสดงเมื่อ `canEdit` เป็นจริง มิฉะนั้นส่ง `null`
  - `settingsFooter` — เมื่อ **ยังไม่ได้ล็อกอิน** (`!signedIn`) จะแสดงข้อความ `t('auth.readOnlyNote')`
    ("เปิดอ่าน SOP ได้โดยไม่ต้องเข้าสู่ระบบ · เข้าสู่ระบบเมื่อต้องการแก้ไขเท่านั้น") ถ้าล็อกอินแล้วส่ง `null`
    เพราะไม่มีอะไรต้องอธิบาย
- **สถานะที่โมดูลนี้แยกแยะยังคงมี 3 สถานะเท่าเดิม** (ยังไม่ล็อกอิน / ล็อกอินแล้ว / ล็อกอินแล้วและเป็น editor)
  เพียงแต่ย้ายที่แสดงเท่านั้น เหตุผลที่ยังต้องบอก "อ่านอย่างเดียว" ให้ชัดคือ **การอ่าน SOP ไม่ได้แปลว่าต้องมี
  session** (ดูหัวข้อ 6) ผู้ใช้ที่เปิดอ่านโดยไม่ล็อกอินจึงควรรู้ว่าที่แก้ไขไม่ได้เป็นเรื่องปกติ ไม่ใช่ระบบเสีย
- `canEdit` ที่ใช้ตรงนี้มาจาก store ซึ่ง **ตั้งค่าเริ่มต้นเป็นเปิดไว้ก่อน** จนกว่า server จะยืนยันเป็นอย่างอื่น
  (เหตุผลอยู่ในหัวข้อ 6) — ป้าย "ผู้แก้ไข" จึงอาจขึ้นชั่วขณะระหว่างโหลดครั้งแรก

**ไฟล์ที่เกี่ยวข้อง:** `src/components/TopBar.jsx`, `src/lib/icons.jsx` (ไอคอน `search`), `src/store.jsx`
(query / setQuery / canEdit / userEmail), `@vcb/shared` (`AppBar`, `useAuth`, `useI18n`)

---

### 2.10 หน้าต้อนรับ (Welcome) — หน้าเริ่มต้นเมื่อยังไม่ได้เลือกอะไร

**ทำอะไรได้:** `src/components/Welcome.jsx` คือสิ่งที่ผู้ใช้เห็นในคอลัมน์ขวาเมื่อ **ยังไม่ได้เลือก case หรือ
flow ใด ๆ** ทำหน้าที่อธิบายว่าเอกสารนี้คืออะไรและใช้อย่างไร ประกอบด้วย 5 ส่วนเรียงจากบนลงล่าง:

1. **หัวเรื่องและคำโปรย** — ใช้ `meta.title` / `meta.subtitle` จากเอกสารจริง
2. **แถบสถิติ (Stat strip)** — แถวชิปคั่นด้วยเส้นแนวตั้ง แสดงเวอร์ชันเอกสาร, วันที่มีผลบังคับใช้, จำนวน case,
   จำนวนรายงาน, จำนวนผังกระบวนการ และชื่อคู่มืออ้างอิง (`meta.manual`)
3. **กล่อง "วัตถุประสงค์"** (`meta.purpose`) พื้นหลังจม
4. **การ์ดวิธีใช้ 3 ใบ** (`HowTo`) — หมายเลข 1/2/3 พร้อมหัวข้อและคำอธิบาย บอกวิธีใช้โครงสามคอลัมน์
5. **กล่องหมายเหตุระดับเอกสาร** (`meta.notes[]`) — กล่องเตือนขอบซ้ายสีเหลือง (`border-warn`) แสดงเป็น bullet

**Logic การทำงาน:**

- **ไม่รับ props เลย** — อ่านทุกอย่างจาก store โดยตรง (`const { meta, scenarios, reports, status } = useStore()`)
  จึงนำไปวางที่ route ไหนก็ได้โดยไม่ต้องส่งค่าอะไรให้
- **การ์ดสถานะโหลด:** ถ้า `status === STATUS.loading` จะคืน `<Spinner />` ออกไปก่อน (early return) ไม่ render
  โครงหน้าที่มีตัวเลข 0 ทั้งแถบ — เพราะแถบสถิติจะอ่านได้ว่า "0 กรณี · 0 รายงาน" ซึ่งดูเหมือนเอกสารว่างเปล่า
  ทั้งที่แค่ยังโหลดไม่เสร็จ
- **ทุกฟิลด์ของ `meta` เป็น optional ทั้งหมด** และมี fallback คนละแบบตามความสำคัญ:
  - `meta.title` / `meta.subtitle` ใช้ `||` fallback ไปที่ข้อความจากพจนานุกรม (`welcome.heading`,
    `welcome.lead`) เพราะหน้าหนึ่งหน้าต้องมีหัวเรื่องเสมอ
  - `meta.version`, `meta.effective`, `meta.manual`, `meta.purpose` และ `meta.notes` ใช้ **conditional render**
    (`{meta.version && …}`) คือถ้าไม่มีค่าก็ **ไม่แสดงชิปหรือกล่องนั้นเลย** ไม่ใช่แสดงชิปเปล่า
  - `meta.notes?.length > 0` ใช้ optional chaining ป้องกันกรณี `notes` เป็น `undefined` ซึ่งเป็นค่าจริงของ
    `EMPTY_DOC` ตอนสถานะ `notSeeded`
- **ตัวเลข 3 ตัวในแถบสถิติมาจากคนละแหล่ง:** `scenarios.length` และ `reports.length` มาจากเอกสารที่ดึงผ่าน API
  ส่วน `SOP_FLOWS.length` **import ตรงจาก `src/data/flows.js`** ไม่ผ่าน store — สอดคล้องกับหลักการในหัวข้อ 2.5
  ที่ว่าผังกระบวนการเป็นข้อมูล static ผลคือแม้เอกสารยังไม่ได้ seed หน้านี้ก็ยังบอกจำนวนผังได้ถูกต้อง
- **คอมโพเนนต์ย่อยภายในไฟล์** (ไม่ export ออกไปข้างนอก เพราะใช้ที่นี่ที่เดียว):
  - `Stat({ children })` — ชิปหนึ่งช่องในแถบสถิติ ใช้ `border-r … last:border-r-0` เพื่อให้เส้นคั่นหายไปเอง
    ที่ชิปสุดท้าย โดยไม่ต้องรู้ล่วงหน้าว่ามีกี่ชิป (ซึ่งไม่คงที่ เพราะหลายชิปเป็น conditional)
  - `HowTo({ n, title, desc })` — การ์ดวิธีใช้หนึ่งใบ ตัวเลขอยู่ในวงกลมสีแบรนด์
- ข้อความของการ์ดวิธีใช้ทั้งหมดมาจากพจนานุกรม (`welcome.ht1Title` … `welcome.ht3Desc`) จึงสลับภาษาได้
- ข้อความภาษาไทยทุกจุดในหน้านี้ใส่ class `break-thai` เพื่อคุมการตัดบรรทัดภาษาไทยไม่ให้ตัดกลางคำ

**จุดที่ควรทราบ:** ใน `App.jsx` หน้านี้ยังถูกใช้เป็น **fallback ของ Process Flows** ด้วย — route `/flows` และ
`/flows/module/:mod` render `Welcome` (ดูหัวข้อ 2.5: ผังที่หา `id` ไม่เจอจะ fallback มาหน้านี้แทนหน้า error)
ต่างจาก route ฝั่ง case/report ที่จะสลับไปเป็น `NotSeeded` เมื่อ `status === STATUS.notSeeded`

**ไฟล์ที่เกี่ยวข้อง:** `src/components/Welcome.jsx`, `src/components/ui.jsx` (`Spinner`),
`src/data/flows.js` (`SOP_FLOWS`), `src/store.jsx` (`STATUS`, `useStore`), `src/App.jsx` (การผูก route)

---

### 2.11 หน้าโหลดข้อมูลไม่สำเร็จ (LoadError)

**ทำอะไรได้:** `src/components/LoadError.jsx` คือหน้าเต็มที่แสดงแทนเนื้อหาทั้งหมดเมื่อ **ดึงเอกสาร SOP ไม่สำเร็จ**
ประกอบด้วยไอคอนเตือนสามเหลี่ยมในวงกลมสีแดง, หัวเรื่อง "โหลดข้อมูลไม่สำเร็จ" (`error.title`), กล่อง `Notice`
โทน `danger` ที่บอกสาเหตุ และปุ่ม "ลองใหม่" (`common.retry`) พร้อมไอคอนรีเฟรช

**Logic การทำงาน:**

- **ต่างจาก `NotSeeded.jsx` โดยเจตนา** (ระบุในคอมเมนต์หัวไฟล์): กรณี `NotSeeded` คือ API **ตอบถูกต้องแล้ว**
  เพียงแต่ยังไม่มีเนื้อหาในฐานข้อมูล ซึ่งเป็นสถานะปกติก่อนนำเข้าข้อมูลจริง ส่วน `LoadError` คือ request
  **ล้มเหลวจริง** — สองสถานะนี้จึงต้องได้หน้าจอคนละหน้า ไม่ใช่หน้าเดียวกัน (ดู `App.jsx`:
  `status === STATUS.error` → `LoadError`, `status === STATUS.notSeeded` → `NotSeeded`)
- **ไม่รับ props** — อ่าน `error` และ `refresh` จาก store
- **ข้อความสาเหตุมาจาก error *code* เสมอ ไม่ใช่จาก `err.message`** — ผ่าน `t(errorKey(error))` โดย `errorKey()`
  ใน `src/lib/sopApi.js` แปลง error object เป็นคีย์พจนานุกรม (`error.<CODE>` และ default เป็น `error.INTERNAL`)
  คอมเมนต์หัวไฟล์อธิบายเหตุผลว่า `shared/src/api.js` กำหนดไว้ชัดเจนว่าฟิลด์ `error` คือ **รหัสสำหรับเครื่องอ่าน**
  ไม่ใช่ข้อความที่เขียนมาให้คนอ่าน — การเอา `err.message` มาแสดงตรง ๆ จะได้ข้อความภาษาอังกฤษเชิงเทคนิคที่ผู้ใช้
  ทำอะไรต่อไม่ได้ และแปลเป็นภาษาไทยไม่ได้ด้วย
- **กรณีพิเศษ 409:** `errorKey()` แปลง status 409 (ที่ไม่ใช่ `NOT_SEEDED`) เป็น `error.CONFLICT` เสมอ ซึ่งอ่านว่า
  "มีคนอื่นแก้ไขพร้อมกัน ให้รีเฟรชแล้วลองใหม่" — สอดคล้องกับกลไก read-modify-write ในหัวข้อ 4.2 และ 5.3
- **ปุ่มลองใหม่** เรียก `refresh` จาก store ซึ่งไป trigger `load()` รอบใหม่ ไม่ใช่ `window.location.reload()` —
  จึงไม่เสียตำแหน่ง route ที่ผู้ใช้อยู่ และไม่ต้องโหลด bundle ใหม่ทั้งก้อน
- ถ้า error code ที่ได้มาไม่มีคีย์รองรับในพจนานุกรม ระบบ `t()` จะคืนคีย์นั้นออกมาเอง ผู้ใช้จะเห็นรหัสดิบ
  ซึ่งยังพอแจ้งทีมพัฒนาต่อได้ ดีกว่าหน้าว่างเปล่า

**ไฟล์ที่เกี่ยวข้อง:** `src/components/LoadError.jsx`, `src/lib/sopApi.js` (`errorKey`), `src/components/ui.jsx`
(`Button`, `Notice`), `src/lib/icons.jsx` (ไอคอน `alert`, `refresh`), `src/store.jsx` (`error`, `refresh`)

---

## 3. ระบบภาษา (i18n) และองค์ประกอบ UI พื้นฐาน

### 3.1 พจนานุกรมคำแปล — `src/i18n/dict.js`

**ทำอะไรได้:** ไฟล์นี้คือ **แหล่งเดียว** ของข้อความทุกคำที่ผู้ใช้เห็นในโมดูล SOP ทั้งภาษาไทยและอังกฤษ ไม่มี
คอมโพเนนต์ไหนเขียนข้อความไทยลงไปตรง ๆ (ยกเว้นเนื้อหา SOP เองที่มาจากฐานข้อมูล) การสลับภาษาในหน้าตั้งค่า
(หัวข้อ 2.8) จึงเปลี่ยนข้อความได้ทั้งแอปทันที

**โครงสร้าง:** เป็น object แบน (flat) คีย์เป็น **dot notation** และค่าเป็นคู่ `{ th, en }` ต่อหนึ่งคีย์
ห่อด้วย `createDictionary()` จาก `@vcb/shared` แล้ว export ทั้งแบบ named (`sopDict`) และ default:

```js
export const sopDict = createDictionary({
  'app.title':        { th: 'ระเบียบปฏิบัติงานมาตรฐาน', en: 'Standard Operating Procedure' },
  'auth.editorBadge': { th: 'ผู้แก้ไข', en: 'Editor' },
  // …
});
```

พจนานุกรมนี้ถูก merge ทับ `commonDictionary` ของระบบผ่าน `<I18nProvider dictionary={sopDict}>` แปลว่าคีย์กลาง
(เช่น `common.close`, `common.loading`, `common.empty`, `common.retry`) ใช้ได้ทันทีโดยไม่ต้องนิยามซ้ำ และถ้า
โมดูลนี้ต้องการเขียนทับคีย์กลางตัวไหน ก็เพียงนิยามชื่อเดียวกันในไฟล์นี้

**กลุ่มคีย์ (แบ่งด้วยคอมเมนต์คั่นในไฟล์):** `chrome` (ชื่อแอป/แบรนด์), `labels`, `reports`, `search`, `list`,
`detail`, `welcome`, `flows`, `case studies`, `settings`, `editor`, `versions`, `not-seeded state`, `errors`
และ `sign-in` รวมประมาณ 150 คีย์ ชื่อกลุ่มตรงกับคอมโพเนนต์ที่ใช้จริง ทำให้ตามหาคีย์จากสิ่งที่เห็นบนหน้าจอได้
โดยไม่ต้องค้นทั้งไฟล์

**Logic และกฎการทำงานที่สำคัญ:**

- **ข้อความภาษาไทยในไฟล์นี้คือ "ถ้อยคำทางธุรกิจฉบับจริง" ห้ามแปลใหม่** — คอมเมนต์หัวไฟล์เขียนกรอบไว้ชัดเจน:
  *"THE THAI HERE IS AUTHORITATIVE BUSINESS COPY. DO NOT RETRANSLATE IT."* ค่า `th` ทุกตัวยกมา **verbatim**
  จาก I18N map ใน `data/config.ts` เดิม ซึ่งเองก็ยกมา verbatim จากแอปต้นฉบับ (Apps Script `index.html`) อีกที
  นี่คือถ้อยคำที่ระเบียบปฏิบัติงานของ MANGO ERP เขียนไว้และเป็นคำที่พนักงานถูกอบรมมา — **แก้ภาษาอังกฤษได้ถ้าอ่าน
  แล้วไม่ลื่น แต่ห้าม "ปรับปรุง" ภาษาไทย**
- **ทำไมจับคู่ th/en ไว้ในคีย์เดียวกัน:** ของเดิมใน `config.ts` เป็น object แยกกันสองก้อน (`I18N.th` / `I18N.en`)
  คีย์เป็นชื่อ camelCase — เมื่อจับสองภาษามาไว้บรรทัดเดียวกัน **คำแปลที่ขาดหายจะเห็นได้ทันทีด้วยตา** ไม่ต้อง
  ไล่เทียบสองก้อน
- **การแทรกค่าตัวแปร (interpolation):** พจนานุกรมเดิมเก็บ **ฟังก์ชัน** สำหรับ 4 ข้อความที่มีตัวเลข (เช่น
  `showingFmt(n, t)`) แต่ `translate()` ของ shared ใช้ placeholder รูปแบบ `{name}` แทน จึงแปลงเป็น `{n}` / `{t}`
  โดย **คงลำดับคำภาษาไทยไว้เหมือนเดิมทุกประการ** เรียกใช้เป็น `t('list.showing', { n, t })` ตัวอย่างเช่น
  `'reports.sub'` = `'เมนูเรียกรายงานสำคัญทั้งหมด · {n} จาก {t} รายการ'` — ข้อดีของ placeholder คือภาษาที่
  เรียงลำดับคำต่างกันย้ายตำแหน่ง `{n}` ได้เองในสตริง โดยไม่ต้องแก้โค้ดที่เรียก
- **คีย์กลุ่ม `error.*` ผูกกับ error code ของ API โดยตรง** — `errorKey()` ประกอบคีย์เป็น `error.<CODE>` ดังนั้น
  ชื่อคีย์อย่าง `error.CONFLICT`, `error.NOT_SEEDED`, `error.SWAP_TARGET_NOT_FOUND`, `error.SWAP_SELF`,
  `error.BAD_SCENARIO_NO`, `error.BAD_CASE_NO`, `error.BAD_VERSION_ID` **ต้องสะกดตรงกับ code ที่ Express ส่งมา
  เป๊ะ ๆ (ตัวพิมพ์ใหญ่ทั้งหมด)** เพิ่ม error code ใหม่ที่ฝั่ง API เมื่อไหร่ ต้องเพิ่มคีย์คู่กันที่นี่ด้วย ไม่งั้น
  ผู้ใช้จะเห็นรหัสดิบบนหน้าจอ (ดูหัวข้อ 2.11) — ส่วนคีย์ที่เป็น camelCase (`error.title`, `error.saveFailed`,
  `error.swapFailed`, `error.deleteFailed`, `error.restoreFailed`, `error.notEditor`) เป็นข้อความของ UI เอง
  ไม่ได้ผูกกับ code ของ server
- คอมเมนต์ที่แทรกไว้ในไฟล์บันทึก **เหตุผลของการเปลี่ยนแปลงเทียบกับของเดิม** ไว้เป็นจุด ๆ เช่น เดิมไม่มีคีย์
  `app.title` เลย ทำให้ `TopBar` ต้องส่ง subtitle ไปใช้ทั้งสองช่อง และ `app.subtitle` เดิมตรึงเป็นข้อความไทย
  ตายตัวไม่ว่าจะเลือกภาษาใด ผลคือผู้อ่านภาษาอังกฤษได้ข้อความไทย ส่วนผู้อ่านภาษาไทยได้ข้อความไทยซ้ำสองครั้ง
- คีย์กลุ่ม `editor.*` ถูกเพิ่มเข้ามาใหม่ เพราะ `EditModal` เดิม **ฮาร์ดโค้ดป้ายภาษาไทยไว้ในตัวเอง** ไม่ผ่าน
  พจนานุกรม ฟอร์มแก้ไขจึงเป็นภาษาเดียว ต่างจากส่วนอื่นของแอป
- คีย์กลุ่ม `not-seeded state` มีอยู่แยกต่างหากตามหลักการเดียวกับหัวข้อ 2.11: `404 NOT_SEEDED` เป็นสถานะปกติ
  ก่อนเปิดใช้งานจริง ไม่ใช่ความล้มเหลว จึงมีชุดข้อความของตัวเอง ไม่ใช้ข้อความ error

**ไฟล์ที่เกี่ยวข้อง:** `src/i18n/dict.js`, `@vcb/shared` (`createDictionary`, `I18nProvider`, `useI18n`,
`translate`), `src/lib/sopApi.js` (`errorKey` — ผู้ประกอบคีย์ `error.*`)

### 3.2 คอมโพเนนต์ UI พื้นฐาน — `src/components/ui.jsx`

ไฟล์รวมคอนโทรลที่โมดูลนี้ใช้ซ้ำ ๆ **เขียนเองด้วยมือทั้งหมด ไม่มี UI kit** ตาม `TECH_STACK.md` — คอมเมนต์หัวไฟล์
ระบุเจตนาไว้ว่ามีเพื่อไม่ให้ปุ่มแบบเดียวกันถูกจัดสไตล์เพี้ยนกันไปคนละแบบใน 6 ไฟล์ **ไม่ได้ตั้งใจให้โตเป็น
component library**

| ที่ export | หน้าที่ |
|---|---|
| `Button` | ปุ่มมาตรฐาน prop `variant` เลือกจาก `VARIANTS`: `default` / `primary` / `danger` และ `type` มีค่าเริ่มต้นเป็น `"button"` (ไม่ใช่ `"submit"`) เพื่อไม่ให้ปุ่มที่อยู่ในฟอร์ม submit โดยไม่ตั้งใจ |
| `Modal` | ฉากหลังทึบ + แผงกลางจอ props: `title`, `onClose`, `dismissable`, `size`, `children`, `footer` โดย `size` map เป็นความกว้าง `sm` / `md` / `lg` / `full` |
| `Field` | ป้ายกำกับ **วางเหนือ** คอนโทรลเสมอ (ไม่ใช่ด้านข้าง) รับ `label` และ `hint` |
| `TextInput` / `TextArea` / `Select` | ช่องกรอกที่ใช้สไตล์ชุดเดียวกัน (ค่าคงที่ `CONTROL`) โดย `TextArea` เพิ่ม `resize-y` |
| `Notice` | กล่องข้อความแบบไม่ขวางการทำงาน prop `tone`: `info` / `warn` / `danger` / `ok` |
| `Empty` | สถานะ "ไม่มีข้อมูล" ถ้าไม่ส่ง children จะใช้ `t('common.empty')` |
| `Spinner` | ตัวหมุนระหว่างโหลด ถ้าไม่ส่ง `label` จะใช้ `t('common.loading')` |
| `BackBar` | แถบ "ย้อนกลับ" สำหรับจอแคบ ถ้าส่ง `to` จะ `navigate(to)` ถ้าไม่ส่งจะ `navigate(-1)` |

**รายละเอียด logic ที่มีเหตุผลเบื้องหลัง:**

- **`Modal` กับ prop `dismissable`:** ค่าเริ่มต้นเป็น `true` แต่ `EditCaseModal` ส่ง `false` เข้ามา เพราะฟอร์มนั้น
  อาจมีข้อความที่พิมพ์ไว้จำนวนมาก การเผลอคลิกโดนฉากหลังแล้วทิ้งงานทั้งหมดเป็นสิ่งที่แอปต้นฉบับก็กันไว้เช่นกัน —
  เมื่อ `dismissable = false` ทั้ง handler คลิกฉากหลัง (`onClick` เป็น `undefined`) และ listener ปุ่ม Escape
  (early return ใน `useEffect`) จะไม่ถูกติดตั้งเลย เหลือทางปิดเพียงปุ่ม × หรือปุ่มใน footer เท่านั้น
  (สอดคล้องกับกฎในหัวข้อ 5.3)
- **การคลิกฉากหลังตรวจ `e.target === e.currentTarget`** — ปิดเฉพาะเมื่อคลิกโดนฉากหลังจริง ๆ ไม่ใช่คลิกในแผงแล้ว
  event bubble ขึ้นมาถึงฉากหลัง
- **`Field` ใช้เลย์เอาต์ป้ายบน–คอนโทรลล่าง** โดยเจตนา คอมเมนต์ระบุว่าเป็นรูปแบบที่ **รอดจากป้ายภาษาไทย** ซึ่งยาว
  กว่าอังกฤษมาก ถ้าวางป้ายไว้ด้านข้างจะบังคับให้คอลัมน์ป้ายกว้างผิดสัดส่วน
- **`Notice` ใช้สีสถานะร่วมของระบบ** (`info` / `warn` / `danger` / `ok` จาก preset) ไม่ใช่สี red/green สำเร็จรูป
  ของ Tailwind ตามที่ note ของ preset กำหนดไว้ และรองรับ dark mode ครบทุกโทน
- **`Empty` ตั้งใจให้ "เงียบ"** — คอมเมนต์ระบุว่า "ไม่มีข้อมูล" คือ *สถานะ* ไม่ใช่ *ความล้มเหลว* จึงใช้ตัวอักษร
  สีจาง ไม่ใช้กล่องเตือน (ต่างจาก `LoadError` ในหัวข้อ 2.11 ซึ่งเป็นความล้มเหลวจริง)
- **`BackBar` ซ่อนตัวเองบนจอกว้างด้วย `lg:hidden`** เพราะบนจอกว้างทั้งสามคอลัมน์แสดงพร้อมกันอยู่แล้ว จึงไม่มี
  "ที่ให้ย้อนกลับไป" และแถบนี้เป็น `sticky top-0` เพื่อให้ยังกดได้แม้เลื่อนเนื้อหาลงไปไกล
- ทุกคอนโทรลรับ `className` เพิ่มเติมจากผู้เรียกและ spread `...rest` ต่อไปยัง element จริง จึงยังส่ง `disabled`,
  `aria-*`, `value`, `onChange` ฯลฯ ได้ตามปกติ

**ไฟล์ที่เกี่ยวข้อง:** `src/components/ui.jsx` — ถูกใช้แทบทุกคอมโพเนนต์ในโมดูล

### 3.3 ชุดไอคอน — `src/lib/icons.jsx`

**ทำอะไรได้:** ให้คอมโพเนนต์ `<Icon name="…" className="h-4 w-4" />` สำหรับวาดไอคอนเส้น (แนว Lucide) แบบ
inline SVG มีทั้งหมด 22 ชื่อ ได้แก่ `workflow`, `clipboard`, `barchart`, `search`, `settings`, `mail`,
`refresh`, `sun`, `moon`, `logout`, `shield`, `edit`, `alert`, `book`, `chevron`, `arrowleft`, `copy`,
`check`, `plus`, `calendar`, `externalLink` และ `link`

**Logic การทำงาน:**

- **ฝังมากับตัวแอปทั้งหมด ไม่มี CDN** — `ICONS` เป็น map จากชื่อไปยังสตริง path ของ SVG ยกมา **verbatim** จาก
  `ICONS` map และ `svgIcon()` ใน `index.html` ของแอปต้นฉบับ
- `Icon` render `<svg viewBox="0 0 24 24">` โดยใส่ path ผ่าน `dangerouslySetInnerHTML` (ปลอดภัยเพราะเนื้อหา
  เป็นค่าคงที่ในไฟล์ ไม่ได้มาจากผู้ใช้หรือฐานข้อมูล)
- **ชื่อไอคอนที่ไม่มีในแมป จะคืน `null`** — ไม่ throw และไม่วาดกล่องว่าง การพิมพ์ชื่อผิดจึงทำให้ไอคอนหายไปเฉย ๆ
  ไม่ทำให้ทั้งหน้าพัง
- **สีมาจาก `currentColor`** (`stroke="currentColor"`, `fill="none"`) ไอคอนจึงรับสีของข้อความที่มันอยู่ข้างใน
  โดยอัตโนมัติ ทั้งโหมดสว่างและมืด ไม่ต้องมีชุดไอคอนแยกต่อธีม
- **ขนาดกำหนดจากฝั่งผู้เรียกด้วย Tailwind class** (เช่น `className="h-4 w-4"`) แทน rule `.ico` แบบเดิม ค่าเริ่มต้น
  คือ `h-[18px] w-[18px]` ซึ่งตรงกับตัวแปร `--ico: 18px` ของ stylesheet เดิม
- ทุกไอคอนตั้ง `aria-hidden="true"` เพราะเป็นภาพประกอบล้วน ๆ ความหมายอยู่ที่ข้อความข้าง ๆ หรือ `aria-label`
  ของปุ่มที่ครอบอยู่ (เช่นปุ่มปิดใน `Modal` และช่องค้นหาใน `TopBar`)

**ไฟล์ที่เกี่ยวข้อง:** `src/lib/icons.jsx` — ใช้ใน `TopBar.jsx`, `LoadError.jsx`, `ui.jsx` (`BackBar`),
`Sidebar.jsx`, `CaseDetail.jsx`, `SettingsModal.jsx` และอื่น ๆ

---

## 4. โครงสร้างข้อมูลและ Data Flow

### 4.1 โมเดลเอกสารเดียว (Single JSONB Document) — ยืนยันจากโค้ด
...
### 4.2 ทำไมต้องเป็น read-modify-write ทั้งก้อน
...
นี่คือที่มาของกฎ "A rejected write must not look like a save" ในหัวข้อ 5.3

### 4.3 `displayNo` — คำนวณฝั่ง server เท่านั้น ไม่เคยถูกจัดเก็บถาวร
...
### 4.4 Endpoints ทั้งหมด (`api/src/routes/sop.js`, base path `/api/sop`)
...
### 4.6 Data Flow สรุปสั้น (ฝั่ง client)
...
## 5. หลักการออกแบบสำคัญ (จากคอมเมนต์ในโค้ดและ PORT_NOTES.md)

### 5.1 ข้อมูลมาจาก API เท่านั้น ไม่มี mock ฝังในแอป
...
### 5.2 Navigation เป็น React Router จริง ไม่ใช่ state จำลอง
...
### 5.3 "A rejected write must not look like a save" (กฎที่สำคัญที่สุดของโมดูลนี้)
...
### 5.4 Drive filename lookup ถูกตัดออกจริง ไม่ใช่แค่ยังไม่ทำ
...
### 5.5 Google Doc mirror ไม่ได้ทำใหม่
...
## 6. สิทธิ์การเข้าถึง (Access Model) — ยืนยันจากโค้ด
